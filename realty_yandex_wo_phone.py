...
with open('realty_yandex_wo_phone.csv', 'w') as csvfile:
    ...
    while count <= 1:
        ...
        #print(html_soup)
        house_data = html_soup.find('ol', class_='OffersSerp__list').findAll('li', class_='OffersSerpItem OffersSerpItem_view_desktop OffersSerpItem_format_full OffersSerp__list-item OffersSerp__list-item_type_offer')
        #print(house_data)
    ...
    while count <= n:
        try:
            ...
            link = 'no infromation'
            link = info.find('a', class_='Link Link_js_inited Link_size_m Link_theme_islands SerpItemLink OffersSerpItem__link')
            link = link.get('href')
            link = 'https://realty.yandex.ru' + link
            #print(link)
            
            count_rooms = 'no infromation'
            count_rooms = info.find('a', class_='Link Link_js_inited Link_size_m Link_theme_islands SerpItemLink OffersSerpItem__link')
